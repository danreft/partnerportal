...
function buildColumns(): ColumnsType<PartnerRow> {
  return [
    {
      title: "Partner",
      key: "name",
    ...
    },
    {
      ...
      render: (v: number, r) => makeLink(v, "leads", r.referralCode),
    },
    {
      title: "Active Deals",
      ...
      render: (v: number, r) => makeLink(v, "active-deals", r.referralCode),
    },
    {
      title: "Stalled",
      dataIndex: "stalled",
      ...
      render: (v: number, r) => makeLink(v, "stalled", r.referralCode, v > 0 ? "#d97706" : undefined),
    },
    {
      title: "Won",
      dataIndex: "won",
      ...
      render: (v: number, r) => makeLink(v, "won", r.referralCode, v > 0 ? "#16a34a" : undefined),
    },
    {
      title: "Lost",
      dataIndex: "lost",
      ...
      render: (v: number, r) => makeLink(v, "lost", r.referralCode, v > 0 ? "#dc2626" : undefined),
    },
    {
      title: "Total Acres",
...
export default function ManagerPage() {
  ...
  const filteredLeads = useMemo(() => {
    if (viewMode === "self" && user?.referralCode) {
      return leadsData.filter((l) => l.referralCode === user.referralCode)
    }
    return leadsData
  }, [viewMode, user])

  const wonAll = filteredLeads.filter((l) => l.stage === "Won")
  const lostAll = filteredLeads.filter((l) => l.stage === "Lost")
  const activeAll = filteredLeads.filter((l) => l.stage !== "Won" && l.stage !== "Lost")
  const stalledAll = filteredLeads.filter(isLeadStalled)

  const TOTALS = {
    referrals: filteredLeads.length,
  ...
  const partnerRows: PartnerRow[] = useMemo(() => {
    const allPartners = viewMode === "self" && user
      ? MOCK_USERS.filter((u) => u.referralCode === user.referralCode)
      : [...partners, ...MOCK_USERS.filter((u) => u.role === "manager" && u.referralCode)]
      ...
      <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", background: "#f9fafb" }}>
        ...
        <main style={{ width: "100%", maxWidth: 1200, margin: "0 auto", flex: 1, padding: 32 }}>
          <Typography.Title level={3} style={{ marginBottom: 24, fontWeight: 600, color: "#111" }}>
            {isOnlyMe ? "My Dashboard" : "Team Dashboard"}
          </Typography.Title>
          ...
          <Row gutter={[24, 24]} style={{ marginTop: 24 }} align="stretch">
            <Col xs={24} lg={16} style={{ display: "flex", flexDirection: "column" }}>
              <StageTable filterReferralCode={isOnlyMe ? user?.referralCode : undefined} />
            </Col>
          ...
          {/* Row 4: Partner Performance (hidden in Only Me mode) */}
          {!isOnlyMe && (
